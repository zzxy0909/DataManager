...
namespace Boomlagoon.JSON {
	...
	public class JSONObject : IEnumerable<KeyValuePair<string, JSONValue>> {
        ...
        public JSONObject GetStringValue(string key)
        {
            var value = GetValue(key);
            if (value == null)
            {
                Debug.LogError(key + "(string) == null");
                return JSONObject.Parse(string.Empty);
            }
            return JSONObject.Parse(value.ToString());
        }
                
        public JSONArray GetStringArray(string key)
        {
            var value = GetValue(key);
            if (value == null)
            {
            ...
            return JSONArray.Parse(value.ToString());
        }
