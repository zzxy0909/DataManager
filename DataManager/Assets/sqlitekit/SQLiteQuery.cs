    ...
    */
    public JSONObject GetRowJson()
    {
        ...
        for (int i = 0; i < columnNames.Length; i++)
        {
            ...
            {
                case (int)Sqlite3.SQLITE_TEXT:
                    obj.Add(columnNames[i], Sqlite3.sqlite3_column_text(vm, i));
                    break;
                case (int)Sqlite3.SQLITE_INTEGER:
                    obj.Add(columnNames[i], Sqlite3.sqlite3_column_int(vm, i));
                    break;
                case (int)Sqlite3.SQLITE_FLOAT:
                    obj.Add(columnNames[i], Sqlite3.sqlite3_column_double(vm, i));
                    break;
                case (int)Sqlite3.SQLITE_BLOB:
                    obj.Add(columnNames[i], Sqlite3.sqlite3_column_text(vm, i));
                    break;
                case (int)Sqlite3.SQLITE_NULL:
                default:
                    obj.Add(columnNames[i], Sqlite3.sqlite3_column_text(vm, i));
                    break;
            }
